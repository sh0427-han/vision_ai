...
section('residual_block', 'Residual block: F(x)와 identity shortcut', asset_figure('resnet-skip.svg', 'CNN residual block에서 입력 x가 convolution branch를 우회해 합산되는 구조', 'ResNet을 CNN의 architecture 확장으로 이해하기 위한 residual block 도식입니다.') + equation('H(x) = F(x) + x\n∂H/∂x = I + ∂F/∂x') + '''<p>예를 들어 x=[2, −1], F(x)=[0.3, 0.2]라면 합산 전 activation은 [2.3, −0.8]입니다. 원 ResNet의 post-activation basic block처럼 합산 뒤 ReLU를 적용하는 경우 [2.3, 0]이 됩니다. Shortcut이 있다고 해서 출력이 항상 입력과 같다는 뜻은 아닙니다.</p><p>또한 <code>∂H/∂x</code>에 identity term이 생긴다는 사실은 gradient가 지나갈 직접 경로를 설명하지만, 이것만으로 모든 깊은 모델의 optimization이 자동으로 해결된다고 해석해서는 안 됩니다.</p>'''),
